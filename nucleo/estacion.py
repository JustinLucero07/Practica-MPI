...
from nucleo.dominio import Medicion, Variable
...
class EstacionAmbiental:
    ...
    def generar_mediciones(self, ciclo: int, proceso: int) -> list[Medicion]:
        mediciones = [
            Medicion(self.nombre, self.zona, var, self._simular(var), ciclo, proceso)
    ...
    def trabajar_ciclo(self, ciclo: int, carga_cpu: int, proceso: int) -> list[Medicion]:
        mediciones = self.generar_mediciones(ciclo, proceso)
        AnalizadorDatos.indice_ambiental(self._historial, carga_cpu)
        return mediciones
